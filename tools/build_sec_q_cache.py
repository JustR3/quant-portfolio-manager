...
def probe_ticker(t: str) -> dict:
    ...
    ni = df[(df["field"] == "net_income") & (df["fiscal_period"].isin(["Q1", "Q2", "Q3"]))]
    if ni.empty:
    ...
    return {"ticker": t, "ok": timely >= 0.5, "why": f"timely 10-Q share {timely:.0%}",
            "q_rows": len(ni), "periods": len(first),
            "first_pe": str(first.index.min().date()), "last_pe": str(first.index.max().date()),
...
def main():
    ...
    args = ap.parse_args()
    set_identity(os.environ.get("EDGAR_IDENTITY", "whispersdi3@gmail.com"))  # SEC-required UA
    tickers = universe()
    if not tickers:
    ...
    for i, t in enumerate(tickers, 1):
        ...
        try:
            ...
            failed += 1
        if i % 25 == 0:
            print(f"[{i}/{len(tickers)}] built={built} skipped={skipped} failed={failed}")
